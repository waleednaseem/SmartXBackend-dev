...
const ADMIN = async (req, res) => {
  ...
  if (!admin) {
    ...
    await User.create({
      username: 'admin',
      password: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwYXNzd29yZCI6ImFkbWluIiwiaWF0IjoxNjgwMTYyNzMzfQ.yOul8XoSF7ygQZagbpyVMNnwjNkMkRcsM-4PcTsXf-w',
      refferal: 0,
      user_id: 1
    })
...
const FIndUserDetail = async (req, res) => {
  ...
  const placement = await Profile.findOne({ where: { [Sequelize.Op.or]: [{ left: user_info.id }, { right: user_info.id }] } });
  const DirectReff = await Profile.findOne({ where: { id: user_info.id }, include: [{ model: Refferal }] })
...
const makeProfile = async (req, res) => {
  ...
  if (pkg == pakage_prices1) {
    ...
    if (findRight3000) {
      ...
      const DirectReff = await Profile.findOne({ where: { id: req.body.refferal } })
      ...
      const ReffkWallets1 = await wallet.findOne({ where: { user_id: req.body.refferal } })
      if (req.body.refferal == 1) {
        await wallet.update({ payment: adminkWallets1.payment + 3000 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
        ...
        await wallet.update({ payment: adminkWallets1.payment + 300 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
        await wallet.update({ payment: ReffkWallets1.payment + 2700 }, { where: { user_id: ReffkWallets1.user_id } }) // 90% for user
        await Transaction.create({
          from: usermake.id,
          to: 1,
    ...
    if (findRight5000) {
      ...
      if (req.body.refferal == 1) {
        await wallet.update({ payment: adminkWallets1.payment + 5000 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
         await Transaction.create({
        from: 'meta mask',
        to: user_info.id,
        reason: 'commision and tax for admin',
        ...
        await wallet.update({ payment: adminkWallets1.payment + 500 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
        await wallet.update({ payment: ReffkWallets1.payment + 4500 }, { where: { user_id: ReffkWallets1.user_id } }) // 90% for user
         await Transaction.create({
        from: user_info.id,
        to: 1,
    ...
    if (findRight10000) {
      ...
      const ReffkWallets1 = await wallet.findOne({ where: { user_id: req.body.refferal } })
      if (req.body.refferal == 1) {
        await wallet.update({ payment: adminkWallets1.payment + 10000 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
        await Transaction.create({
          from: 'meta mask',
          to: user_info.id,
          reason: 'commision and tax for admin',
        ...
        await wallet.update({ payment: adminkWallets1.payment + 1000 }, { where: { user_id: adminkWallets1.user_id } }) // 10% for admin
        await wallet.update({ payment: ReffkWallets1.payment + 9000 }, { where: { user_id: ReffkWallets1.user_id } }) // 90% for user
        await Transaction.create({
          from: user_info.id,
          to: 1,
          reason: 'tax for admin',
...
const Placements = async (req, res) => {
  ...
  const user = await Profile.findOne({ where: { id: req.body.id }, include: [{ model: Refferal, as: 'Refferal', include: [{ model: Profile, as: 'directReff' }] }] })
...
const FindUserPakage = async (req, res) => {
  const user = req.headers.authorization.split(' ')[1]
  const user_info = jwt_decode(user)
  const refferal = await Refferal.findAll({ where: { refferal: user_info.id }, include: [{ model: Profile, as: 'ReffUsers', include: [{ model: Pakage }] }] })
...
const findTransac = async (req, res) => {
  const user = req.headers.authorization.split(' ')[1]
  const user_info = jwt_decode(user)
  const transaction = await Transaction.findAll({ where: { user_id: user_info.id } })
  res.json(transaction)
}

const decode = async (req, res) => {
  try {
    const user = await Profile.findOne({ where: { username: req.body.username } });
